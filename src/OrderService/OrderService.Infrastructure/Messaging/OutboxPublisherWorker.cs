    ...
    ILogger<OutboxPublisherWorker> logger) : BackgroundService
{
    ...
    private async Task ProcessOutboxMessagesAsync()
    {
        ...
        var handlers = scope.ServiceProvider.GetRequiredService<IEnumerable<IOutboxMessageProducer>>();
        var processedMessages = new List<OutboxMessage>();

        foreach (var message in messages)
        {
            var producer = handlers.FirstOrDefault(h => h.MessageType == message.Type);
            ...
            try
            {
                await producer.PublishAsync(message.Payload, message.Id);
